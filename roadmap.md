...
- [x] **ステップ4：手首の位置にモデルを追従させる**
  - ステップ1で修正した座標（`wristPosRial`）を、剣のモデルの `position` に毎フレーム代入する。

- [x] **ステップ5：法線ベクトルと指のベクトルで回転（Rotation）を制御する**
...
**handtracking側のループ処理内でのnewなくす
オブジェクト指向終わったら、汎用化(任意のモデルをドラックするだけで、簡易的なトラッキングができるようにする。既存のelucidatorなどは、サイトで選択できるようにしておいてそれも今まで通りできるようにする。)**
